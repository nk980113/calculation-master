...
export default function LevelBox({ id }: {
    ...
    return (
        <div className='level-box' onClick={() => setLevel(id)}>
            <div className={`level-title ${level.difficulty ? 'hard' : ''}`}>{level.title + '👿'.repeat(level.difficulty)}</div>
            ...
            <div className='max-question-count'>{accStarCount > 1 ? '⭐'.repeat(questionStarCount) + '★'.repeat(3 - questionStarCount) + `${raw.maxQuestionCount.toFixed(3)}題/秒` : '🔒尚未解鎖'}</div>
        </div>
    );
}
